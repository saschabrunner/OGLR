...
void main()
{
    ...
    if (lightFragmentAngle > light.cutoff) // '>' because we have cos values, so values decrease from 0 to 90 degrees
    {
    ...
    {
        color = vec4(light.ambient * vec3(texture(material.diffuse, textureCoordinates)), 1.0);
    }
}